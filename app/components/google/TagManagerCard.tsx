              ...
              <div>
                ...
                <div className="flex items-center gap-4 text-sm">
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    Last updated {data.container.lastUpdated}
                  </span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="text-2xl font-bold text-gray-900">
                  {data.performance.totalTags}
                </div>
                {getChangeIndicator(data.performance.tagChange)}
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="text-2xl font-bold text-gray-900">
                  {data.performance.activeTags}
                </div>
                <p className="text-xs text-gray-500 mt-1">
                  {((data.performance.activeTags / data.performance.totalTags) * 100).toFixed(1)}% of total
                </p>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm text-gray-600">Firing Rate</span>
                </div>
                <div className="text-2xl font-bold text-gray-900">
                  {data.performance.firingRate.toFixed(1)}%
                </div>
                {getChangeIndicator(data.performance.firingRateChange)}
              </div>
                ...
                {data.recentChanges.map((change, index) => (
                  <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                      change.type === 'error' ? 'bg-red-100' :
                      change.type === 'create' ? 'bg-green-100' :
                      change.type === 'update' ? 'bg-blue-100' : 'bg-yellow-100'
                    }`}>
                      {change.type === 'error' && <AlertTriangle className="h-4 w-4 text-red-600" />}
                      {change.type === 'create' && <Plus className="h-4 w-4 text-green-600" />}
                      {change.type === 'update' && <Edit className="h-4 w-4 text-blue-600" />}
                      {change.type === 'publish' && <Play className="h-4 w-4 text-yellow-600" />}
                    </div>
                ...
                <tbody className="divide-y divide-gray-200">
                  {data.tags.map((tag) => (
                    <tr key={tag.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2 text-sm text-gray-700">{tag.firingCount}</td>
                      <td className="px-4 py-2 text-sm text-gray-700">{tag.lastFired}</td>
                      <td className="px-4 py-2 text-sm">
                        <div className="flex items-center gap-2">
                          <button
              ...
              {data.triggers.map((trigger) => (
                <div key={trigger.id} className="border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="text-sm text-gray-700 mb-2">
                    <p>Fires on: {trigger.condition}</p>
                    <p>Used by: {trigger.usedBy} tags</p>
                  </div>
